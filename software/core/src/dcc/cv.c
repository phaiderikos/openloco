...
bool is_cv_implemented(uint16_t num)
{
    ...
    if (num == 1u) {
        impl = true;
    } else if (num == 3u || num == 4u) {        /* Acceleration & Deceleration Rate */
        impl = true;
    } else if (num == 7u || num == 8u) {        /* Manufacturer Version No. & Manufactured ID */
        impl = true;
    } else if (num == 13u || num == 14u) {      /* Alternate Mode Function Status F1-F8 & FL,F9-F12 */
        impl = true;
    } else if (num == 17u || num == 18u) {      /* Extended Address */
        impl = true;
    } else if (num == 27) {                     /* Decoder Automatic Stopping Configuration */
        impl = true;
    } else if (num >= 33 && num <= 46) {        /* Output Locations 1-14 for Functions FL(f), FL(r), and F1-F12 */
        impl = true;
    } else if (num == 65) {                     /* Kick Start */
        impl = true;
    } else if (num >= 67 && num <= 94) {        /* Speed Table */
        impl = true;
    }
