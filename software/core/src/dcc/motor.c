...
#define MAX_PWM_DC_VALUE_16BIT      (0xFFFFu)
#define SPEED_STEPS_COUNT           (28u)       /**< Standard amount of speed steps as defined in DCC specifications */
#define ACCEL_FACTOR                (896u)      /**< Acceleration factor for CV3 and CV4 */
#define PWM_MAX_DIVIDER             (64u)
#define PWM_TIM_MAX_VALUE           (1023u)

static uint8_t speed_table[SPEED_STEPS_COUNT];
...
uint32_t configure_motor(motor * const me, QEvt const * const e)
{
     ...
     */
    me->accel_t = (pCfgEvt->config.acc_rate * ACCEL_FACTOR) / SPEED_STEPS_COUNT;  /* 896 / 28 = 32 */
    me->decel_t = (pCfgEvt->config.dec_rate * ACCEL_FACTOR) / SPEED_STEPS_COUNT;
    ...
    if (temp_pwm_t < 1u) {
        temp_pwm_t = 1u;
    } else if (temp_pwm_t > PWM_MAX_DIVIDER) {
        temp_pwm_t = PWM_MAX_DIVIDER;
    }

    /* Calculate pwm timer ARR (with timer clock @ 32MHz) */
    me->pwm_arr = ((PWM_TIM_MAX_VALUE + 1u) * temp_pwm_t) - 1u;
    ...
    Q_ENSURE((PWM_TIM_MAX_VALUE <= me->pwm_arr) && (me->pwm_arr <= 0xFFFFu));
...
void motor_pwm_set(int16_t const value, enum motor_direction const dir)
{
    /* Inside DRV8872 datasheet, it's suggested to use a PWM on one channel and a static one (break) on the other */
    if (dir == DIRECTION_FORWARD) {
        BSP_SetMotorPwm1(value);
        BSP_SetMotorPwm2(MAX_PWM_DC_VALUE_16BIT);
    }
    else {
        BSP_SetMotorPwm1(MAX_PWM_DC_VALUE_16BIT);
        BSP_SetMotorPwm2(value);
    }
}

void brake(bool const fast)
{
    if (fast) {
        /* IN1 and IN2 both high -> Brake (low-side slow decay) */
        BSP_SetMotorPwm1(MAX_PWM_DC_VALUE_16BIT);
        BSP_SetMotorPwm2(MAX_PWM_DC_VALUE_16BIT);
    }
    else {
        /* IN1 and IN2 both low -> Coast (H-bridge disabled to High-Z) */
        BSP_SetMotorPwm1(0u);
        BSP_SetMotorPwm2(0u);
    }
}
