...
static QSpyId const SysTickISR = { 0U };
#endif
...
static void configure_clock(void)
{
	...
	volatile uint32_t i;
	for (i = 0u; (i < (SIMPLE_DELAY_2MS)) && ((__HAL_RCC_GET_FLAG(RCC_FLAG_PLLRDY)) == 0u); i++) {
		/* Wait PLL Ready bit ... */
	}
	...
	/* Right now there are no timing sources available, wait in a simple way */
	for (i = 0u; (i < (SIMPLE_DELAY_2MS)) && (__HAL_FLASH_GET_LATENCY() != FLASH_LATENCY_1); i++) {
		/* Wait PLL Ready bit ... */
	}
	if (i >= (SIMPLE_DELAY_2MS)) {
		/* TIMEOUT! */
	...
	/* Right now there are no timing sources available, wait in a simple way */
	for (i = 0u; (i < (SIMPLE_DELAY_2MS)) && (__HAL_RCC_GET_SYSCLK_SOURCE() != RCC_SYSCLKSOURCE_STATUS_PLLCLK); i++) {
